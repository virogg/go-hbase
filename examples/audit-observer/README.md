...
Пример post-hook audit: Go-observer, который эмитит одну структурированную
...
- **Log-based observability**: записи проходят через `slog` как JSON
  на stderr; bridge пересылает их в лог RegionServer, готовые для любого
  log-агрегатора.
- **Приватность payload**: записи audit несут короткий SHA-256 digest
