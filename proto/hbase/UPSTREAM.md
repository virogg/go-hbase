...
| `Client.proto` | `Client.proto` | Урезано: `MutationProto` (с вложенными `ColumnValue`, `QualifierValue`, `Durability`, `MutationType`, `DeleteType`) плюс read-путь `Get`, `Result`, `Scan` (и `Cursor`, `Column`, `Filter`, `Comparator`, `Consistency`, `ReadType`), добавленный для reverse-RPC канала endpoint'ов. RPC-обёртки запросов/ответов (`MutateRequest`/`Response`, `GetRequest`, `ScanRequest` …) не вендорятся: обратный канал несёт голые `Get`/`Scan`/`MutationProto`. |
