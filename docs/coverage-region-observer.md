...
# Матрица покрытия RegionObserver
...
|---------------|----------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| **dispatch**  | паритет через рефлексию (каждый hook в `hookTable` ↔ метод `RegionObserver` ↔ enum `HookId`) | `pkg/hbasecop/hooktable_test.go::TestHookTableIsCanonical`, `TestRegionObserverInterfaceCoversAllHooks` |
| **roundtrip** | wire round-trip: каждый `proto.Request` кодируется / декодируется без потерь                 | `internal/wire/hookpb/*` (на основе corpus) и Java `wire.pb.*RoundTripTest`                             |
| **prePutIT**  | живой IT (Put → счётчик Go observer)                                                         | `test/java/.../PrePutCounterIT.java`, `examples/counter-observer/`                                      |
| **readIT**    | живой IT (Get / Scan bypass)                                                                 | `test/java/.../ReadPathFilterIT.java`, `examples/filter-observer/`                                      |
| **batchIT**   | живой IT (частичный блок `Table.batch`)                                                      | `test/java/.../BatchPartialBlockIT.java`                                                                |
| **storageIT** | живой IT (`admin.flush` + `admin.majorCompact`)                                              | `test/java/.../StorageHooksIT.java`                                                                     |
| **faultIT**   | матрица fault-injection                                                                      | `test/java/.../FaultMatrixIT.java`                                                                      |
...
- **roundtrip**: proto Request сериализуется идентично с обеих сторон
  ...
  ещё нет.
- **dispatch**: через рефлексию утверждает, что строка hook в `hookTable`
...
WALRestore, внутренности MemStoreCompaction), намеренно покрыты на уровне
`dispatch` + `roundtrip`, а не выделенным живым IT: эти серверные пути
срабатывают редко, и поднимать их детерминированно в IT непропорционально
дорого. Выделенные per-surface адаптеры observer-типов
(`bridge.observer.*ObserverAdapter`), маршрутизирующие эти hooks, реализованы;
зафиксированы reflection-паритет диспетчеризации и кросс-языковой wire-контракт.
